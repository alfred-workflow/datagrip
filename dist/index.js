...
class Alfred {
    constructor() {
        this.params = process.argv.filter((_v, index) => index > 1);
        this.input = process.argv.length > 2 ? process.argv[process.argv.length - 1] : '';
    }
    get env() {
        return process.env;
    }
    output(output, inputMatch) {
        var _a;
        const input = (_a = this.input) === null || _a === void 0 ? void 0 : _a.toLocaleLowerCase();
        const inputArr = this.input.toLocaleLowerCase().split('|').filter(i => i);
        console.log(JSON.stringify(Object.assign(Object.assign({}, output), { items: output.items.filter(i => (inputMatch && input) ?
...
class Jetbrains {
    ...
    getOptionsFileObjSync(app, fileName) {
    ...
    async getOptionsFileObj(app, fileName) {
    ...
    resolveFileContent(file) {
        const recentPreferencesObj = lib.xml2js(file);
        return this.resolveElement(recentPreferencesObj);
    }
    replaceStringValue(value) {
    ...
    getRecentProjectsManagerSync(app) {
        return this.getOptionsFileObjSync(app, 'recentProjects.xml');
    }
    async getRecentProjectsManager(app) {
        return this.getOptionsFileObj(app, 'recentProjects.xml');
    }
}
let jetbrains = new Jetbrains();

const items = [{ arg: '', title: 'Open', subtitle: '' }];
try {
    const recentProjectsManager = jetbrains.getRecentProjectsManagerSync('DataGrip');
    items.push(...[...recentProjectsManager.additionalInfo.entries()].reverse().map(([key, value]) => {
        ...
        return ({
            ...
            title: `${(_b = (_a = value.frameTitle) === null || _a === void 0 ? void 0 : _a.split(' –')[0]) !== null && _b !== void 0 ? _b : key.slice(key.lastIndexOf('/') + 1)}${value.opened === 'true' ? '(opened)' : ''}`,
            subtitle: key
        });
    }));
}
